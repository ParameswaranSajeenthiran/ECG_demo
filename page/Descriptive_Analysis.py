...
class DescriptiveAnalysis(Page):
    ...
    def content(self):
        ...
        total2, total3 = st.columns(2, gap='medium')
        ...
        with col1:
            p_start = info['ECG_P_Peaks'][0] - 10 if 'ECG_P_Peaks' in info and info['ECG_P_Peaks'] else 0
            t_start = info['ECG_T_Peaks'][0] + 10 if 'ECG_T_Peaks' in info and info['ECG_T_Peaks'] else len(last_rows)
            ...
            def extract_peak_coordinates(peak_key, x_range_start, x_range_end):
                peak_coordinates = []
                if peak_key in info and len(info[peak_key]) > 0:
                    valid_indices = [i for i in info[peak_key] if
                                     i >= x_range_start and i <= x_range_end and i < len(last_rows)]
            ...
            if p_peak_coordinates and q_peak_coordinates and r_peak_coordinates and s_peak_coordinates and t_peak_coordinates:
                fig_segment.add_shape(type="rect", xref="x", yref="paper", x0=p_peak_coordinates[0][0], y0=0,
                                      x1=q_peak_coordinates[0][0], y1=1, fillcolor="green", opacity=0.2)
                fig_segment.add_shape(type="rect", xref="x", yref="paper", x0=q_peak_coordinates[0][0], y0=0,
                                      x1=s_peak_coordinates[0][0], y1=1, fillcolor="yellow", opacity=0.2)
                fig_segment.add_shape(type="rect", xref="x", yref="paper", x0=s_peak_coordinates[0][0], y0=0,
                                      x1=t_peak_coordinates[0][0], y1=1, fillcolor="red", opacity=0.2)

                fig_segment.add_annotation(x=(s_peak_coordinates[0][0] + t_peak_coordinates[0][0]) / 2, y=1.1,
                                           text="ST Interval", showarrow=False, font=dict(color="black", size=12))
                fig_segment.add_annotation(x=(p_peak_coordinates[0][0] + q_peak_coordinates[0][0]) / 2, y=1.1,
                                           text="PQ Interval", showarrow=False, font=dict(color="black", size=12))
                fig_segment.add_annotation(x=(q_peak_coordinates[0][0] + s_peak_coordinates[0][0]) / 2, y=1.1,
                                           text="QRS Complex", showarrow=False, font=dict(color="black", size=12))
            else:
                st.warning("Not enough data to display ECG intervals. Please ensure that the signal contains valid peaks.")
